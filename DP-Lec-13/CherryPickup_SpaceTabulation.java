...
public class CherryPickup_SpaceTabulation {
     ...
     int[][] prev = new int[m][m];
      ...
      for(int j1=0;j1<m;j1++){
         for(int j2=0;j2<m;j2++){
            if(j1 == j2)
            prev[j1][j2] = matrix[n-1][j1];
            else
            prev[j1][j2] = matrix[n-1][j1]+matrix[n-1][j2];
         }
      }
   ...
   for(int i =n-2;i>=0;i--){
       ...
       for(int j1=0;j1<m;j1++){
         for(int j2=0;j2<m;j2++){
            ...
            int nextMove = ((j1+d1 < 0 || j1+d1 >= m) || (j2+d2 < 0 || j2+d2 >= m)) ?  (int) -1e9 : prev[j1+d1][j2+d2] ;
